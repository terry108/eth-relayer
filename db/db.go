...
func (w *BoltDB) UpdateBridgeHeight(h uint32) error {
	w.rwlock.Lock()
	defer w.rwlock.Unlock()
	...
	return w.db.Update(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(BKTHeight)
		return bkt.Put([]byte("bridge_height"), raw)
	})
}

func (w *BoltDB) GetBridgeHeight() uint32 {
	w.rwlock.RLock()
	defer w.rwlock.RUnlock()
	...
	_ = w.db.View(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(BKTHeight)
		raw := bkt.Get([]byte("bridge_height"))
		if len(raw) == 0 {
			h = 0
			return nil
		}
